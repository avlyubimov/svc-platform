...
| Current telemetry | Current telemetry trace, INA228 pin template, board-current budget trace, 40 A freeze review, ADC scaling, filtering, calibration plan, and total-current monitor choice |
...
| CAN1 safety | CAN1 TX-disable trace, production DNP review, DNP/open TX path, default disable state, status readback, DNP BOM ownership, firmware listen-only behavior, and future ADR hardware-action process |
