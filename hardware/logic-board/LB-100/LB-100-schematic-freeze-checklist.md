...
| Service, storage, and sensors | Closed | `hardware/logic-board/LB-100/LB-100-requirements.md`, `hardware/logic-board/LB-100/LB-100-service-storage-sensor-precheck.csv`, `hardware/logic-board/LB-100/LB-100-component-sourcing-precheck.csv`, `hardware/logic-board/LB-100/LB-100-fb-powered-off-corrective-review-2026-07-21.md`, `hardware/logic-board/LB-100/LB-100-e73-recovery-interface.md` | USB-C service path, BLE, microSD, RTC, FRAM, IMU, lux sensor, ESD, and powered-off isolation are selected; E73 UART/reset remain below VDD + 0.3 V when RADIO_SENSOR_3V3 is off and five accessible module recovery pads expose SWDIO/SWDCLK/reset/target-reference/GND |
...
| KiCad schematic review | Closed | `hardware/logic-board/LB-100/kicad/LB-100.kicad_sch`, `hardware/logic-board/LB-100/kicad/lib/LB100.kicad_sym`, `hardware/logic-board/LB-100/LB-100-schematic-review-closeout.md`, `tools/validate_board_schematics.py`, `docs/adr/ADR-0020-platform-evt-development-and-reference-loads.md`, `docs/adr/ADR-0021-fog-switch-common-and-protection-ownership.md` | Deterministic value-bearing capture exports 104 components and 199 electrical nets; PA8 `FOG_A_SW_IN` and PA9 `FOG_B_SW_IN` have independent selectable dry/12 V secondary-conditioning, pull-up, RC, clamp and Schmitt-buffer chains while PB-100 owns primary cable-entry protection; exact E73 powered-off isolation and recovery access plus all prior topology typed IC pins and symbol/footprint pads pass; ERC has no errors and only the reviewed cross-board USB_CC1/USB_CC2 isolated-label warnings |

## Release Boundary

`LB-100-board-release-blocker-register.csv` has zero active LBREL blockers.
Schematic freeze and mechanical pre-layout evidence are closed. ADR-0020 has
since authorized controlled `.kicad_pcb` placement and routing, while the
CAN/USB/SD/clock/BLE signal-integrity and safety review remains open. No Gerber,
drill, placement, or manufacturing package is approved by this freeze.
