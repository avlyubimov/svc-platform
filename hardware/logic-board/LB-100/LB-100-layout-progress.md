...
- Generated the controlled four-layer `kicad/LB-100.kicad_pcb` from the
  reviewed 83-component schematic and project-local footprints.
...
- Placed MCU, rail conversion, JFB1, service interfaces, microSD, BLE, IMU,
  lux sensor, ten-channel control/IMON support and the `FOG_SW_IN` filter in
  the reviewed functional zones.
...
- Schematic parity is exact apart from H1-H8, which are board-only mechanical
  holes. The 349 unconnected findings are the explicit routing backlog.

## EVT-FAB Blockers

- Route all 349 remaining connections and add reviewed reference planes.
...
- Review ten IMON ADC paths, INA228 I2C/addressing, `VBAT_SENSE`,
  `FOG_SW_IN`, boot safe-state, connector orientation and supplier DFM.
