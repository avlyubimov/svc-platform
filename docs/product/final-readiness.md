...
| PB-100 PCB/layout | EVT-LAYOUT-AUTHORIZED | There is 1 active blocker: `PBREL-007`, which remains production-only under ADR-0020. The controlled eight-layer 150 mm x 90 mm layout places all 414 footprint-bound schematic components plus eight intentional board-only mounting holes. Its deterministic baseline contains 5,677 segments, 910 conventional through vias, 39 adjacent-layer 0.30/0.10 mm microvias and 38 zones. KiCad 10.0.4 refill DRC reports zero errors and zero unconnected items; schematic parity contains only H1-H8, and the 188 remaining findings are locked non-electrical silkscreen/library warnings. Implementation and connectivity evidence for `PB-EVT-FAB-001` and `PB-EVT-FAB-002` is complete, but their checklist status remains Open until formal EVT-FAB review. The aggregate authorization remains `EVT-LAYOUT-AUTHORIZED`. Fabrication is still blocked pending 40 A electrothermal, switch/clamp-loop, rail and CAN review; connector and laboratory-safety evidence; supplier acceptance of sequential lamination, stacked microvias and filled/capped U3 via-in-pad; DFM/yield/quote review within the EVT budget; and Product Owner approval. No manufacturing outputs are authorized, and production remains `NO-GO` |
...
| LB-100 PCB/layout | EVT-FAB-AUTHORIZED | There are 0 active blockers in the release register (0 active LBREL blockers). The controlled six-layer 100 mm x 70 mm board places all 104 schematic components plus eight holes and carries 2,274 deterministic segments and 409 standard 0.50/0.30 mm through vias. Four GND zones use F.Cu/In2.Cu/In4.Cu/B.Cu and the exact E73 rule area covers all six copper layers. KiCad 10.0.4 refill DRC has zero errors and zero unconnected items; parity contains only H1-H8 and 27 locked non-electrical warning waivers. USB uses In3.Cu over In2.Cu with 0.20 mm width, approximately 0.157 mm edge gap, 1.112 mm skew and an approximately 85 ohm analytical guard; exact supplier 90-ohm field-solver confirmation is an order preflight. The Product Owner authorized a segregated five-board bare-PCB EVT package. Physical FFC fit, BLE enclosure range, bench evidence and production/Rev.2 qualification remain open. PB-100 and the combined order remain `NO-GO` |
...
| Production package | Draft | `production/board-order/three_board_jlcpcb_order_readiness.csv` keeps PB-100, FB-100 and the combined platform `NO-GO`; only the segregated five-piece LB-100 Rev.1 EVT bare-PCB package is authorized, and it is explicitly not a production or assembly release |
