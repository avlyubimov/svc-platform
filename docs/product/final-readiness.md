...
| PB-100 PCB/layout | EVT-LAYOUT-AUTHORIZED | There is 1 active blocker: `PBREL-007`; ADR-0020 makes it a production-only blocker. The controlled four-layer 150 mm x 90 mm partial import places all 28 currently footprint-bound schematic components plus eight holes. Only the complete CAN1 safety island is routed (84 segments, eight vias, nine `CAN1_*` nets), with no shorts/crossings/clearance findings; CI forbids non-CAN nets in that partial routing manifest. It is explicitly not fabricable: 46 missing footprints, six U1 parity findings, 126 unconnected items, all 40 A/power copper and the complete diagnostic population remain open. The aggregate authorization remains `EVT-LAYOUT-AUTHORIZED`. Q2-C100 is paused and optional. `EVT-FAB-AUTHORIZED` still requires the separate PB EVT-fab checklist, zero DRC/unconnected/parity findings, routed 40 A electrothermal and clamp-loop estimates, connector fit, DFM and laboratory safety evidence. Production remains `NO-GO` |
| LB-100 requirements | Frozen | Baseline is frozen by ADR-0014 and the schematic freeze is Closed |
| LB-100 KiCad schematic | Reviewed | Deterministic 100-component, 199-net, footprint-bound capture adds independent protected PA8 `FOG_A_SW_IN` and PA9 `FOG_B_SW_IN` chains with mutually exclusive dry-contact/12 V populations, typed IC pins/ERC, sourced and decoupled ADC_REF, one-point AGND return, digital USB VBUS detection, direct STM32-to-LTC3212 drive, back-power-safe sensors, and E73 UART/reset isolation; exported-netlist audit and ERC pass with only the two reviewed cross-board USB CC single-pin warnings |
| LB-100 PCB/layout | EVT-LAYOUT-AUTHORIZED | There are 0 active blockers (0 active LBREL blockers). The controlled four-layer 100 mm x 70 mm board places all 100 schematic components plus eight holes and now carries a deterministic first routing iteration with 1,860 segments and 172 vias. KiCad reports no shorts, crossings or copper-clearance violations; 55 of the original 384 connections remain open around MCU/JPB1 and critical interfaces. Reference planes, BLE keepout, dual-input connector/protection review, USB/SI/safety review and footprint/silkscreen cleanup remain open; manufacturing outputs remain blocked |
...
| FB-100 PCB/layout | EVT-LAYOUT-AUTHORIZED | There are 0 active blockers (0 active FBREL blockers). The controlled 80 mm x 35 mm board contains all 44 frozen schematic footprints plus four mounting holes and a deterministic connectivity-complete route with 438 segments and 39 vias. KiCad reports zero unconnected items, shorts, crossings and copper-clearance violations. Ground pours, stackup-derived USB impedance/gap/length tuning, five courtyard overlaps, three silkscreen clips, USB shell edge-entry review, optional FOG-button fit and final fab review remain open. No Gerber, drill, BOM/CPL, pick-place, manufacturing ZIP, or PCBA order exists |
