...
| LB-100 KiCad schematic | Reviewed | Deterministic 99-component, 199-net, footprint-bound capture adds independent PA8 `FOG_A_SW_IN` and PA9 `FOG_B_SW_IN` secondary-conditioning chains with mutually exclusive dry-contact/12 V populations while ADR-0021 moves primary cable-entry suppression to PB-100; typed IC pins/ERC, sourced and decoupled ADC_REF, one-point AGND return, digital USB VBUS detection, direct STM32-to-LTC3212 drive, back-power-safe sensors, and E73 UART/reset isolation remain; exported-netlist audit and ERC pass with only the two reviewed cross-board USB CC single-pin warnings |
| LB-100 PCB/layout | EVT-LAYOUT-AUTHORIZED | There are 0 active blockers (0 active LBREL blockers). The controlled four-layer 100 mm x 70 mm board places all 99 schematic components plus eight holes and carries a deterministic routing iteration with 1,869 segments and 176 vias. KiCad reports no shorts, crossings or copper-clearance violations; 53 connections remain open around MCU/JPB1 and critical interfaces. Both raw FOG paths now reach JPB1, while `FOG_B_SW_IN_MCU`, reference planes, BLE keepout, USB/SI/safety review and footprint/silkscreen cleanup remain open; manufacturing outputs remain blocked |
