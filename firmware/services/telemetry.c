...
void svc_telemetry_snapshot_init(svc_telemetry_snapshot_t *snapshot)
{
    ...
    for (size_t zone_index = 0U; zone_index < SVC_THERMAL_ZONE_COUNT; ++zone_index) {
        snapshot->thermal_c[zone_index] = 0;
        snapshot->thermal_updated_ms[zone_index] = 0U;
        snapshot->thermal_valid[zone_index] = false;
    }
}
...
bool svc_telemetry_update_thermal(
    ...
    uint32_t now_ms)
{
    if (snapshot == NULL || !thermal_zone_is_valid(zone)) {
        return false;
    }

    const uint8_t zone_index = (uint8_t)zone;
    snapshot->thermal_c[zone_index] = temperature_c;
    snapshot->thermal_valid[zone_index] = valid;
    snapshot->thermal_updated_ms[zone_index] = now_ms;
    return true;
}
    ...
    uint32_t stale_after_ms)
{
    ...
    const uint8_t zone_index = (uint8_t)zone;
    return snapshot->thermal_valid[zone_index] &&
           timestamp_is_fresh(snapshot->thermal_updated_ms[zone_index], now_ms, stale_after_ms);
}
...
{
    if (snapshot == NULL || !thermal_zone_is_valid(zone)) {
        return (svc_telemetry_thermal_input_t){0, false};
    }

    return (svc_telemetry_thermal_input_t){
        .measured_temperature_c = snapshot->thermal_c[(uint8_t)zone],
        .telemetry_valid = svc_telemetry_thermal_is_valid(snapshot, zone, now_ms, stale_after_ms)
    };
}
